...
class Parser(object):
    ...
    def parse(self, response, parser):
        if parser['type'] == 'xpath':
            return self.xpathParse(response, parser)
        elif parser['type'] == 'regular':
            return self.regularParse(response, parser)
    ...
    def xpathParse(self, response, parser):
        ...
        for proxy in proxys:
            try:
                ...
                country = ""
                area = ""
                if '省' in addr or self.AuthCountry(addr):
                    country = '国内'
                    area = addr
                else:
                    country = '国外'
    ...
    def regularParse(self, response, parser):
        '''
                针对正则表达式进行解析
                :param response:
                :param parser:
    ...
    def CnproxyPraser(self, response, parser):
        proxylist = self.regularParse(response, parser)
    ...
    def proxy_listPraser(self, response, parser):
        ...
        if matchs:
            for match in matchs:
                try:
                    ip_port = base64.b64decode(match.replace("Proxy('", "").replace("')", "")).decode()
                    ip = ip_port.split(':')[0]
